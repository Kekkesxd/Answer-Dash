...
async function protectGamePage() {
  try {
...
leaderboardButton.addEventListener("click", async () => {
  ...
  try {
    ...
    const leaderboard = Array.isArray(result)
      ? result
      : result.leaderboard || result.scores || [];
    ...
    leaderboardList.innerHTML = renderLeaderboardHTML(leaderboard);
  } catch (error) {
  ...
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      zones.push({
        ...
        vx: currentSettings?.movingZones
          ? (Math.random() < 0.5 ? -1 : 1) * currentSettings.zoneSpeed
          : 0,
        vy: currentSettings?.movingZones
          ? (Math.random() < 0.5 ? -1 : 1) * currentSettings.zoneSpeed
          : 0,

        shrinkRate: currentSettings?.movingZones
          ? currentSettings.zoneShrinkRate
          : 0,
...
function generateObstacles() {
  ...
  while (obs.length < obstacleCount && spacing >= 0) {
    ...
    while (obs.length < obstacleCount && attempts < maxAttemptsPerSpacing) {
      ...
      const tooCloseToOtherObstacle = obs.some((o) =>
        rectsOverlap(candidate, o, spacing)
      );
    ...
    .map((entry, index) => {
      return `
        <div class="leaderboard-row">
          ...
          <span>${entry.player_name || entry.username || "Player"}</span>
...
async function loadEndLeaderboard() {
  endLeaderboardList.innerHTML = "Loading...";
  ...
  try {
    ...
    endLeaderboardList.innerHTML = renderLeaderboardHTML(leaderboard);
  } catch (error) {
    console.error("Could not load end leaderboard:", error);
    endLeaderboardList.innerHTML = "Could not load leaderboard.";
  }
}
...
async function endGame() {
  ...
  finalText.innerHTML = `${playerName}, you scored <span style="color:#00ffb4">${score}</span> point(s)!<br><br>
  ...
  for (const [i, z] of zones.entries()) {
    ...
    if (resultCorrectIndex !== null) {
      if (i === resultCorrectIndex) {
        ...
        i === resultChosenIndex &&
        resultChosenIndex !== resultCorrectIndex
      ) {
    ...
    ctx.shadowBlur = 10;
    wrapText(
      currQuestion ? currQuestion.answers[i] : z.label,
      z.x + z.w / 2,
      z.y + z.h / 2,
      z.w - 20,
      18
...
function drawQuestionBox() {
  ...
  ctx.shadowBlur = 10;
  wrapText(
    currQuestion ? currQuestion.question : "Loading....",
    canvas.width / 2,
    qb.y + qb.h / 2,
    qb.w - 40,
    22
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {
    ...
    timerDisplay.style.textShadow =
      timeLeft <= 3 ? "0 0 10px #ff2d6f" : "0 0 10px #00ffb4";
